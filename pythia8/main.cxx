// Use this program to create files with combinatorial combinations
// S.Chekanov (ANL)
// 2023 (May)
...
int main(int argc, char* argv[]) {
	...
	// fastjet
	const double ptLepton=20;
	const double ptJet=20;
	const double etaJet=2.5;
	const double Rsize = 0.4; // jet size 0.4
	const double misRateEle=0; // Use 0%  // 1%
	const double misRateMu=0;   // Use 0 // 0.1%
	// https://arxiv.org/pdf/1512.01094.pdf
	//1+0.003*pT  (~1000 GeV is about 4%)
	const double misRateB=0;   // 0% for this study. 1% b-tag misidentification rate
	...
	// record combinations
	// TMap *Combinations = new TMap();

	map<string, int> combinations;
	map<string, int> combinations_process;
	...
	*/
	...
	for (int n = 0; n < Ntot; n++) {
		...
		// process code
		int m_run=(int)pythia.info.code();
		int m_event=n;
		...
		*/
		...
		for (int i =0; i<pythia.event.size(); i++) {
			...
			if (abs(type) ==11 || abs(type) ==13 || abs(type) ==22 || abs(type) ==15) {
				...
				if (pt>ptLepton && TMath::Abs(eta)<etaLep) {
					...
					//if (abs(type) ==15) cout << "ID=" << abs(type) << " status="<< status << " eta=" << eta << endl;
					candidates.push_back(p); // all but tau
				}
		...
		//TObjString *s2 = new TObjString("OK");
		//Combinations->Add((TObject*)s1, (TObject*)s2);


		//TString *name;
		//name->Form("%d",jets.size());
		//TString *value = new TString("OK");
		//Combinations->Add((TObject*)name, (TObject*)value);


		// jets
		string strMET = "0";
		if (met_pt>200)  strMET = "1";


		// jets
		std::string strJ = std::to_string(jets.size());

		// bjets
		std::string strB = std::to_string(bjets.size());

		// elec
		std::string strE = std::to_string(electrons.size());

		// muons
		std::string strM = std::to_string(muons.size());

		// photons
		std::string strP = std::to_string(photons.size());

		// tau
		std::string strT = std::to_string(taus.size());

		string key = strMET+" "+strJ+" "+strB+" "+strE+" "+strM+" "+strP+" "+strT;


		if (combinations.find(key) == combinations.end()) {
			combinations[key] = 1; // not found
		} else {
			combinations[key] = combinations[key]+1;
		}


		// now include Pythia process too
		string keyP = std::to_string(m_run) + " "+ key;

		if (combinations_process.find(keyP) == combinations_process.end()) {
			combinations_process[keyP] = 1; // not found
		} else {
			combinations_process[keyP] = combinations_process[keyP]+1;
		}
	...
	*/
	...
	// write MAP
	string xfile="combinations.txt";
	cout << "Write=" << xfile << endl;
	WriteFile(xfile, &combinations);

	xfile="combinations_process.txt";
	cout << "Write=" << xfile << endl;
	WriteFile(xfile, &combinations_process);
